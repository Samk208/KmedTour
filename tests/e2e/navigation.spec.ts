...
test.describe("Core Navigation", () => {
  ...
  test("coordinator redirects to login when not authenticated", async ({
    page,
  }) => {
  ...
  test("health endpoint returns ok/503", async ({ request }) => {
    const response = await request.get("/api/health");
    expect([200, 503]).toContain(response.status());
    const body = await response.json();
    // API shape: { ok: boolean, timestamp: string, checks: Record<string, boolean|string> }
