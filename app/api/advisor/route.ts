...
const advisorInputSchema = z.object({
  ...
  language: z.array(z.string().min(1)).min(1).max(10),
})
...
export async function POST(req: Request) {
  ...
  if (rateLimitResponse) {
    logger.warn('Advisor rate limit exceeded', {
      path: '/api/advisor',
      method: 'POST',
    })
  ...
  try {
    ...
    const { symptoms, duration, budget, language } = advisorInputSchema.parse(json)
    ...
    const result = await streamObject({
      ...
      schema: z.object({
        ...
        reasoning: z.string().describe('A brief, empathetic explanation of the overall analysis'),
      }),
...
5. Be empathetic but professional.`,
    ...
    logger.error('Advisor request failed', {
      path: '/api/advisor',
      method: 'POST',
    }, {
    ...
    return NextResponse.json(
      { success: false, message: 'Unable to process advisor request' },
      { status: 500 },
    )
  }
}
