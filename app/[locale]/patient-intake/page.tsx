...
export default function PatientIntakePage() {
  ...
  const handleStep3Submit = form3.handleSubmit((data) => {
    ...
    submitIntake(fullData, {
      onSuccess: (response) => {
        if (!response.success) {
          setSubmitError(response.message ?? (t('patientIntakePage.errors.serverError') || 'Something went wrong. Please try again.'))
          return
        }
      ...
      onError: () => {
        setSubmitError(t('patientIntakePage.errors.networkError') || 'We couldn\'t reach the server. Please check your connection and try again.')
      },
    })
  })
        ...
        {currentStep < 4 && (
          <div className="mb-12">
            <h1 className="text-3xl md:text-4xl font-bold text-center mb-4" style={{ color: 'var(--kmed-navy)' }}>
              {t('patientIntakePage.title') || "Patient Intake Form"}
            </h1>
            <p className="text-center text-lg" style={{ color: 'var(--deep-grey)' }}>
              {t('patientIntakePage.subtitle') || "Help us understand your needs to find the perfect clinic match"}
            </p>
          </div>
        )}
          ...
          {currentStep === 1 && (
            <>
              ...
              <div className="flex justify-end pt-8 border-t mt-8" style={{ borderColor: 'var(--border-grey)' }}>
                ...
                >
                  {t('patientIntakePage.buttons.next') || "Next Step"}
            ...
            <>
              ...
              <div className="flex justify-between pt-8 border-t mt-8" style={{ borderColor: 'var(--border-grey)' }}>
                <Button size="lg" variant="outline" onClick={handleBack}>
                  ...
                  {t('patientIntakePage.buttons.back') || "Back"}
                </Button>
                <Button
                  size="lg"
                ...
                >
                  {isPending ? (
                    <>
                      ...
                      {t('patientIntakePage.buttons.submitting') || "Submitting..."}
                    </>
                  ) : (
                    <>
                      {t('patientIntakePage.buttons.submit') || "Submit Application"}
                      <ArrowRight className="ml-2 h-5 w-5" />
                    </>
                  )}
                </Button>
