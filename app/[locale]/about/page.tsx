...
export default function AboutPage() {
  ...
  const values = [
    {
      icon: Shield,
      title: t('aboutPage.values.items.item1.title') || 'Verified Excellence',
      description: t('aboutPage.values.items.item1.description') || 'We strictly vet every clinic against the highest international standards (JCI, K-Health).',
    },
    {
      icon: Cpu,
      title: t('aboutPage.values.items.item2.title') || 'Digital Precision',
      description: t('aboutPage.values.items.item2.description') || 'Our proprietary Medical OS organizes your entire journey with algorithmic efficiency.',
    },
    {
      icon: Heart,
      title: t('aboutPage.values.items.item3.title') || 'Human Touch',
      description: t('aboutPage.values.items.item3.description') || 'While our tech is advanced, our care is personal. Dedicated coordinators for every patient.',
    },
    {
      icon: Globe,
      title: t('aboutPage.values.items.item4.title') || 'Global Access',
      description: t('aboutPage.values.items.item4.description') || 'Democratizing access to Korea\'s elite medical infrastructure for patients worldwide.',
    },
  ]

  const stats = [
    { number: t('aboutPage.stats.stat1.number') || '500+', label: t('aboutPage.stats.stat1.label') || 'Successful Journeys' },
    { number: t('aboutPage.stats.stat2.number') || '50+', label: t('aboutPage.stats.stat2.label') || 'Elite Partners' },
    { number: t('aboutPage.stats.stat3.number') || '20+', label: t('aboutPage.stats.stat3.label') || 'Countries Served' },
    { number: t('aboutPage.stats.stat4.number') || '99%', label: t('aboutPage.stats.stat4.label') || 'Systems Uptime' },
  ]
          ...
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            <div className="space-y-8">
              <div className="space-y-4">
                <div className="text-teal-600 font-bold tracking-wider text-sm uppercase">{t('aboutPage.mission.badge') || "Our Mission"}</div>
                <h2 className="text-3xl md:text-4xl font-bold text-slate-900">
                  {t('aboutPage.mission.title') || "Decoding Complex Care"}
                </h2>
              </div>

              <div className="prose prose-lg text-slate-600">
                <p>
                  {t('aboutPage.mission.p1') || "Medical travel has historically been fragmented, opaque, and risky. Patients worldwide struggle to find trustworthy prices, verified doctors, and safe logistics."}
                </p>
                <p>
                  {t('aboutPage.mission.p2a') || "We built KmedTour not just as an agency, but as a "} <span className="text-teal-700 font-semibold">{t('aboutPage.mission.p2Highlight') || "technology platform"}</span>{t('aboutPage.mission.p2b') || ". By digitizing the patient journey—from AI-powered matching to secure digital health records—we eliminate friction and restore trust."}
                </p>
                <p>
                  {t('aboutPage.mission.p3') || "Today, we serve patients from North America, Europe, Asia, and beyond, proving that high-quality healthcare knows no borders."}
                </p>
              </div>
              ...
              <div className="flex gap-4 pt-4">
                <div className="flex flex-col gap-1">
                  <div className="flex items-center gap-2 font-semibold text-slate-900">
                    <Activity className="w-5 h-5 text-teal-500" /> {t('aboutPage.mission.points.point1.title') || "Data-Driven"}
                  </div>
                  <p className="text-sm text-slate-500">{t('aboutPage.mission.points.point1.description') || "Treatments based on clinical outcomes, not marketing."}</p>
                </div>
                <div className="flex flex-col gap-1">
                  <div className="flex items-center gap-2 font-semibold text-slate-900">
                    <Database className="w-5 h-5 text-blue-500" /> {t('aboutPage.mission.points.point2.title') || "Transparent"}
                  </div>
                  <p className="text-sm text-slate-500">{t('aboutPage.mission.points.point2.description') || "Clear pricing and direct hospital billing access."}</p>
                </div>
              </div>
            </div>
      ...
      <section className="w-full py-24 bg-slate-50 border-y border-slate-200">
        <div className="container mx-auto max-w-[1240px] px-4 sm:px-6">
          <div className="text-center mb-16">
            <h2 className="text-3xl font-bold text-slate-900 mb-4">{t('aboutPage.values.header') || "Core Architecture"}</h2>
            <p className="text-lg text-slate-500 max-w-2xl mx-auto">
              {t('aboutPage.values.subtitle') || "Our operating principles for a flawless medical journey"}
            </p>
          </div>
          ...
          <div className="bg-gradient-to-r from-teal-600 to-blue-700 rounded-3xl p-12 text-center text-white relative overflow-hidden shadow-2xl">
            ...
            <div className="relative z-10 space-y-8 max-w-2xl mx-auto">
              <h2 className="text-3xl md:text-5xl font-bold tracking-tight">
                {t('aboutPage.cta.title') || "Initialize Your Journey"}
              </h2>
              <p className="text-lg text-teal-100">
                {t('aboutPage.cta.subtitle') || "Use our AI Treatment Advisor or browse our curated network of clinics to begin."}
              </p>
              <div className="flex flex-col sm:flex-row gap-4 justify-center">
                <Link href="/treatment-advisor">
                  <Button size="lg" className="bg-white text-teal-700 hover:bg-teal-50 border-0 w-full sm:w-auto font-bold">
                    <Sparkles className="w-4 h-4 mr-2" /> {t('aboutPage.cta.primaryBtn') || "Ask AI Advisor"}
                  </Button>
                </Link>
                <Link href="/patient-intake">
                  <Button size="lg" variant="outline" className="border-teal-200 text-teal-100 hover:bg-teal-700/50 hover:text-white w-full sm:w-auto">
                    {t('aboutPage.cta.secondaryBtn') || "Start Consultation"}
                  </Button>
                </Link>
              </div>
            </div>
